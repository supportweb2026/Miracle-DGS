...
function SidePanelTopContent({ config, formElements, withUpload }) {
  ...
  const isContractEntity = config.entity === 'contract';
  console.log('isContractEntity:',isContractEntity);
  const { entity, ENTITY_NAME } = config; //tizi
  ...
  return (
    <>
      <Row style={show} gutter={(24, 24)}>
        ...
        {isContractEntity && (
            <Button
            onClick={() => handleDownload(currentItem)} // Passer l'objet `record` ici
            type="primary"
...
function FixHeaderPanel({ config }) {
  ...
  const addNewItem = () => {
    if (config.entity === 'contract') {
      // Rediriger vers la page de création dédiée pour les contrats
    ...
    <Row gutter={8}>
      ...
      <Col className="gutter-row" span={3}>
        <Button onClick={addNewItem} block={true} icon={<PlusOutlined />}>
          {config.entity === 'contract' ? 'Nouveau' : ''}
        </Button>
      </Col>
    </Row>
  );
