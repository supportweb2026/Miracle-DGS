...
function AddNewItem({ config }) {
  ...
  // Mapping des entités vers leurs routes correctes
  const getEntityRoute = (entity) => {
    ...
    return routeMapping[entity] || entity.toLowerCase();
  };
...
export default function DataTable({ config, extra = [] }) {
  ...
  const handleRead = (record) => {
    ...
    const route = getEntityRoute(entity);
    navigate(`/${route}/read/${record._id}`);
  };
  const handleEdit = (record) => {
    const data = { ...record };
    dispatch(erp.currentAction({ actionType: 'update', data }));
    const route = getEntityRoute(entity);
    navigate(`/${route}/update/${record._id}`);
  };
