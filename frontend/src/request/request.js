...
const request = {
  ...
  list: async ({ entity, options = {} }) => {
    try {
      includeToken();
      console.log('token:',includeToken());
      let query = '?';
      for (var key in options) {
        query += key + '=' + options[key] + '&';
      }
  ...
  listAllNotifications: async (userId) => {
    try {
      includeToken();
      console.log('🔔 listAllNotifications called with userId:', userId);
  
      const query = userId ? `?user=${encodeURIComponent(userId)}` : '';
      const response = await axios.get(`notification/listAll${query}`);
   ...
   executePowerShellScript: async ({ email, name }) => {
    try {
      includeToken();
      console.log('champs:',email,name);
      const response = await axios.post('notification/executescript', { email, name });
